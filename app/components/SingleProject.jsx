...
const SingleProject = ({ project }) => {
  ...
  // Retourne soit un objet importé, soit une string (public path ou url), soit placeholder
      ...
      <div className='flex flex-col items-center justify-between w-full h-full'>
        ...
        <div className="p-6">
          <Image
            ...
            // Si imageSrc est une URL externe et que tu n'as pas ajouté le domaine
            // tu peux ajouter temporairement unoptimized={true} — mais privilégie next.config.js.
          />
        </div>

        <div className="flex items-center justify-between w-full">
          <Link href={demo || '#'} target='_blank' className="flex justify-center items-center w-10 h-10 rounded-full border-2 border-[#EFF3F4] text-[#EFF3F4] transition-all duration-300 hover:bg-[#231d4b] hover:text-violet-600 hover:border-[#0F0C41] hover:scale-110 decoration-clone cursor-pointer no-underline delay-[0.3s]">
            <FaPlay />
          </Link>

          <Link href={code || '#'} target='_blank' className="flex justify-center items-center w-10 h-10 rounded-full border-2 border-[#EFF3F4] text-[#EFF3F4] transition-all duration-300 hover:bg-[#231d4b] hover:text-violet-600 hover:border-[#0F0C41] hover:scale-110 cursor-pointer no-underline delay-[0.3s] group-hover:translate-x-[-140px]">
            <FaCode />
          </Link>
        </div>
      </div>
